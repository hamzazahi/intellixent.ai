...
<Steps titleSize="h2">
  ...
  <Step title="Configure Your SIP Account (Outbound)">
    ...
    Note the **SIP account number** (e.g., `12345`) — this is your **username** that you will use in Intellixent. Set a password in both the **Password** and **Confirm Password** fields — you will use this same password when importing the number to Intellixent.
    ...
    Scroll down to find your region and copy the **SIP URI** from the regional gateway list. You will need this for the outbound settings in Intellixent.
  ...
  <Step title="Import Number to Intellixent">
    Go to your Intellixent dashboard → **Your Phone Numbers** → **"Integrate SIP trunk"** and fill in your details:
...
<AccordionGroup>
  <Accordion title="Outbound calls not working?">
    ...
    - **Check your password** — Ensure the password in Intellixent matches exactly what you set in the DIDLogic SIP account settings.
  ...
  <Accordion title="Inbound calls not working?">
    ...
    - **Check the IP whitelist in Intellixent.* — Make sure the IP address you added in the Intellixent inbound settings matches the regional gateway you are using. Refer to the gateway IP table in Step 4.
  </Accordion>
</AccordionGroup>
