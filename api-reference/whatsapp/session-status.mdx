...
```bash cURL
curl -X GET "https://portal.intellixent.ai//api/user/whatsapp/session-status?sender_id=12&recipient_phone=+1234567890" \
...
const response = await fetch(
  `https://portal.intellixent.ai//api/user/whatsapp/session-status?${params}`,
  {
    headers: {
...
response = requests.get(
    'https://portal.intellixent.ai//api/user/whatsapp/session-status',
